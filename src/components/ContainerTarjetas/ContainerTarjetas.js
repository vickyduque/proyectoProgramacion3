...
export default class ContainerTarjetas extends Component {
    ...
    // Siempre el llamado a apis tiene que estar en un container y ese container hace el llamado despues a cada una de las tarjetas individualmente.
    componentDidMount() {
        fetch('https://api.themoviedb.org/3/movie/popular?api_key=274e06a482f13c5a152ff7abe7a3142a&language=en-US&page=1')
            ...
            //Con this.state estamos llamando a ese estado, y dentro esta la propiedad "tarjetas" que contiene la informacion a mappear
        ...
        return (
            <div>
                {this.state.tarjetas.map((tarjeta, index) => {
                        ...
                        title={tarjeta.title}
                        backdrop_path={tarjeta.poster_path}
                        overview = {tarjeta.overview}
                        release_date =  {tarjeta.release_date}
                        removerPelicula = {(title) => this.removerPelicula(title)} //paso una funcion mediante props para que elimine la peli. Recibe el id de cada peli. 
                        />
                    })
                }
